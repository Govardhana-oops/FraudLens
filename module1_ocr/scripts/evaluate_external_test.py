"""Dedicated External Unseen-Data Benchmark Evaluation Script for AI-DIDSS Module 1 (Stage 7).

Evaluates the FROZEN LayoutAware-MultiScale-OCR-v2.0 pipeline on the isolated external test dataset.
Strict Rule: No model weights, thresholds, or extraction rules are modified during or after this run.
"""
...
def run_external_benchmark(base_dir: str):
    data_dir = os.path.join(base_dir, "data")
    ann_path = os.path.join(data_dir, "annotations", "external_test_annotations.jsonl")
    img_dir = os.path.join(data_dir, "external_test")

    if not os.path.exists(ann_path):
        print(f"Error: External annotation file '{ann_path}' not found.", file=sys.stderr)
        sys.exit(1)
    ...
    doc_pipeline = DocumentUnderstandingPipeline(min_confidence=FROZEN_THRESHOLD)
    ...
    print("=" * 80)
    print("AI-DIDSS MODULE 1: STAGE 7 DEDICATED EXTERNAL UNSEEN-DATA BENCHMARK")
    print(f"Dataset: SynthID-Doc-External v1.0 | Samples: {len(records)} | Threshold: {FROZEN_THRESHOLD}")
    print("=" * 80)
    ...
    correct_class = sum(confusion_matrix[t][t] for t in ALL_DOC_TYPES if t in confusion_matrix)
    doc_acc = correct_class / total_samples if total_samples > 0 else 0.0
    ...
    ext_results = {
        ...
        "dataset_name": "SynthID-Doc-External v1.0",
    ...
    # Load Internal Test Metrics from Stage 6 for Generalization Gap Calculation
    stage6_metrics_path = os.path.join(base_dir, "STAGE6_METRICS.json")
    gen_gap = {}
    if os.path.exists(stage6_metrics_path):
        with open(stage6_metrics_path, "r", encoding="utf-8") as fp:
            s6_data = json.load(fp)
            s6_test = s6_data.get("test_evaluation", {})
            int_f1 = s6_test.get("field_metrics", {}).get("mean_f1", 0.0)
            int_em = s6_test.get("field_metrics", {}).get("mean_exact_match", 0.0)
            int_acc = s6_test.get("document_classification", {}).get("accuracy", 0.0)
            int_lat = s6_test.get("ocr_metrics", {}).get("mean_latency_ms", 0.0)

            ext_f1 = ext_results["field_metrics"]["mean_f1"]
            ext_em = ext_results["field_metrics"]["mean_exact_match"]
            ext_acc = ext_results["document_classification"]["accuracy"]
            ext_lat = ext_results["ocr_metrics"]["mean_latency_ms"]

            gen_gap = {
                "field_f1_gap": round(int_f1 - ext_f1, 4),
                "exact_match_gap": round(int_em - ext_em, 4),
                "classification_accuracy_gap": round(int_acc - ext_acc, 4),
                "latency_delta_ms": round(ext_lat - int_lat, 2),
                "internal_field_f1": int_f1,
                "external_field_f1": ext_f1,
                "assessment": "Stable Generalization (Gap < 5%)" if abs(int_f1 - ext_f1) < 0.05 else "Moderate Generalization Gap"
            }

    # Save artifacts
    metrics_path = os.path.join(base_dir, "STAGE7_EXTERNAL_METRICS.json")
    cm_path = os.path.join(base_dir, "STAGE7_CONFUSION_MATRIX.json")
    gap_path = os.path.join(base_dir, "STAGE7_GENERALIZATION_GAP.json")

    with open(metrics_path, "w", encoding="utf-8") as fp:
        json.dump(ext_results, fp, indent=2)
    with open(cm_path, "w", encoding="utf-8") as fp:
        json.dump(confusion_matrix, fp, indent=2)
    with open(gap_path, "w", encoding="utf-8") as fp:
        json.dump(gen_gap, fp, indent=2)

    print(f"\n[EXTERNAL TEST RESULTS]: Field F1={ext_results['field_metrics']['mean_f1']*100:.2f}% | ExactMatch={ext_results['field_metrics']['mean_exact_match']*100:.2f}% | Latency={ext_results['ocr_metrics']['mean_latency_ms']:.1f}ms | Throughput={ext_results['ocr_metrics']['throughput_fps']} fps")
    print(f"[DOC CLASSIFICATION]: Accuracy={ext_results['document_classification']['accuracy']*100:.2f}%")
    print(f"[GENERALIZATION GAP]: Field F1 Gap={gen_gap.get('field_f1_gap', 0.0)*100:+.2f}% | Assessment={gen_gap.get('assessment', 'N/A')}")
    print("=" * 80)
    print("Saved Artifacts: STAGE7_EXTERNAL_METRICS.json, STAGE7_CONFUSION_MATRIX.json, STAGE7_GENERALIZATION_GAP.json")
    print("=" * 80)

if __name__ == "__main__":
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    run_external_benchmark(base_dir)
